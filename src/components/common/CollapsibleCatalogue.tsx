...
export const CollapsibleCatalogue: React.FC<CollapsibleCatalogueProps> = ({
  ...
  return (
    <section className={`bg-white border border-stone-200/90 rounded-3xl overflow-hidden shadow-xs transition-all ${className}`}>
      
      {/* Catalogue Header Bar */}
      <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gradient-to-r from-stone-50/90 via-white to-slate-50/70 border-b border-stone-100">
        
        {/* Title & Count Badge */}
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-2xl bg-slate-100 border border-slate-200/80 text-slate-700 flex items-center justify-center font-black shrink-0 shadow-2xs">
            {icon || <Layers className="w-4 h-4 text-slate-600" />}
          </div>

          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="text-sm sm:text-base font-black text-[#0f233a] tracking-tight">
                {title}
              </h3>

              {count !== undefined && (
                <span className="px-2.5 py-0.5 rounded-full bg-slate-100 border border-slate-200/80 text-slate-700 text-[11px] font-extrabold tracking-wide">
                  {countDisplay}
                </span>
              )}
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-1.5 cursor-pointer shadow-2xs select-none active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#026fc3] focus-visible:ring-offset-1 ${
              isOpen
                ? 'bg-slate-900 hover:bg-black text-white'
                : 'bg-[#026fc3] hover:bg-[#025ea6] text-white'
            }`}
          >
            {isOpen ? (
              <>
