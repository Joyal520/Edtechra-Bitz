    ...
    <div className="w-full space-y-4 pt-1">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-1 text-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="px-2.5 py-0.5 rounded-full bg-sky-100 dark:bg-sky-950/70 text-[#026fc3] dark:text-sky-300 font-bold text-xs">
      ...
      <div className="space-y-2">
        <textarea
          ...
          className={`w-full p-4 sm:p-5 rounded-2xl border text-sm sm:text-[15px] font-medium leading-relaxed transition-all focus:ring-2 focus:ring-[#026fc3] focus:border-[#026fc3] focus:outline-none selection:bg-sky-500 selection:text-white ${
            isAnswerLocked
              ? 'bg-stone-50/80 dark:bg-stone-900/60 border-stone-200 dark:border-stone-800 text-slate-800 dark:text-slate-100 cursor-not-allowed'
              : 'bg-white text-slate-900 placeholder:text-stone-400 border-stone-200 dark:bg-[#182232] dark:text-white dark:placeholder:text-stone-400 dark:border-slate-700 caret-[#026fc3] dark:caret-white shadow-2xs'
          }`}
        />
        ...
        {!isAnswerLocked && (
          <div className="flex items-center justify-between gap-3 pt-1">
            ...
            <button
              ...
              className="px-6 py-3 rounded-2xl bg-[#026fc3] hover:bg-[#03589e] text-white text-xs font-black transition-all cursor-pointer disabled:opacity-40 flex items-center gap-2 shadow-sm"
            >
              {isEvaluating ? (
                <>
      ...
      {evaluation && (
        <div className="p-5 sm:p-7 rounded-3xl bg-linear-to-b from-sky-50/70 to-white dark:from-stone-900 dark:to-stone-950 border border-sky-200/90 dark:border-stone-800 shadow-md space-y-5 animate-in fade-in zoom-in-95 duration-300">
          
          {/* Header & Overall Score */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-stone-200/70 dark:border-stone-800">
            <div className="space-y-1">
              <span className="text-[11px] font-black uppercase tracking-wider text-[#026fc3] flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-3 bg-white dark:bg-stone-900 px-4 py-2 rounded-2xl border border-sky-200/80 dark:border-stone-700 shadow-2xs self-start sm:self-auto">
          ...
          {/* Feedback Paragraph */}
          <div className="p-4 rounded-2xl bg-white/90 dark:bg-stone-900/90 border border-stone-200/70 dark:border-stone-800 text-xs sm:text-sm text-slate-800 dark:text-slate-100 leading-relaxed font-medium">
            {evaluation.feedback}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs sm:text-sm">
            ...
            <div className="p-4 rounded-2xl bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-300 dark:border-emerald-800 space-y-2">
              <span className="font-black text-emerald-950 dark:text-emerald-200 flex items-center gap-1.5 text-xs uppercase tracking-wider">
                <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                <span>Key Strengths</span>
              </span>
              <ul className="space-y-1 text-emerald-950 dark:text-emerald-100 font-medium">
            ...
            <div className="p-4 rounded-2xl bg-amber-50/70 dark:bg-amber-950/30 border border-amber-300 dark:border-amber-800 space-y-2">
              <span className="font-black text-amber-950 dark:text-amber-200 flex items-center gap-1.5 text-xs uppercase tracking-wider">
                <AlertCircle className="w-4 h-4 text-amber-600" />
                <span>Improve Next Time</span>
              </span>
              <ul className="space-y-1 text-amber-950 dark:text-amber-100 font-medium">
            ...
            <div className="space-y-2.5 pt-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                {Object.entries(evaluation.criteria_scores).map(([critName, scoreVal]) => (
                  <div key={critName} className="p-3 rounded-xl bg-white dark:bg-stone-900 border border-stone-200/80 dark:border-stone-800 space-y-1.5">
