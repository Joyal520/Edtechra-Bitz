...
export const CourseRoadmap: React.FC<Props> = ({
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-3 sm:px-6 py-6 sm:py-8 space-y-6">
      
      {/* ------------------------------------------------------------------ */}
      {/* 1. COURSE PROGRESS HEADER                                          */}
      {/* ------------------------------------------------------------------ */}
      <div className="p-5 sm:p-6 rounded-3xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 shadow-sm space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <span className="text-[10px] font-black text-[#026fc3] dark:text-sky-400 uppercase tracking-widest block">
              Course Progress
            </span>
          ...
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 rounded-full bg-sky-50 dark:bg-sky-950/60 text-[#026fc3] dark:text-sky-300 text-xs font-black border border-sky-200 dark:border-sky-800">
        ...
        <div className="space-y-1.5">
          <div className="w-full h-3 rounded-full bg-stone-100 dark:bg-stone-800 overflow-hidden border border-stone-200/60 dark:border-stone-700/60">
            <div
      ...
      {/* ------------------------------------------------------------------ */}
      <div className="space-y-3 relative before:absolute before:left-6 sm:before:left-7 before:top-6 before:bottom-6 before:w-0.5 before:bg-stone-200 dark:before:bg-stone-800 before:z-0">
          ...
          return (
            <div
              ...
              className={`relative z-10 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border transition-all duration-200 flex items-start gap-3.5 sm:gap-4 ${
                isCompleted
                  ? 'bg-white dark:bg-stone-900/90 border-emerald-300 dark:border-emerald-900/60 shadow-2xs cursor-pointer hover:border-emerald-500'
                  : isAvailable
                  ? 'bg-white dark:bg-stone-900 border-[#026fc3] dark:border-sky-500 shadow-md ring-2 ring-[#026fc3]/10 cursor-pointer hover:shadow-lg scale-[1.01]'
                  : 'bg-stone-50/80 dark:bg-stone-900/40 border-stone-200 dark:border-stone-800/80 opacity-75 cursor-not-allowed'
              }`}
            >
              ...
              <div className="shrink-0 pt-0.5">
                {isCompleted ? (
                  ...
                  <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-stone-200 dark:bg-stone-800 text-slate-500 dark:text-slate-400 flex items-center justify-center border border-stone-300 dark:border-stone-700">
              ...
              <div className="flex-1 min-w-0 space-y-1">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {/* Status Badge */}
                  {isCompleted && (
                    <span className="px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 text-[10px] font-black uppercase tracking-wider border border-emerald-200 dark:border-emerald-800">
                      ✓ Completed
                    </span>
                  )}
                  {isAvailable && (
                    <span className="px-2 py-0.5 rounded-full bg-sky-50 dark:bg-sky-950/60 text-[#026fc3] dark:text-sky-300 text-[10px] font-black uppercase tracking-wider border border-sky-200 dark:border-sky-800">
                      ▶ {isActive ? 'In Progress' : 'Continue'}
                    </span>
                  )}
                  {isLocked && (
                    <span className="px-2 py-0.5 rounded-full bg-stone-100 dark:bg-stone-800 text-slate-600 dark:text-slate-400 text-[10px] font-black uppercase tracking-wider border border-stone-200 dark:border-stone-700">
