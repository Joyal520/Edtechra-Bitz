                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {!isTeacher && isCompleted && (
                          <span className="text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200 flex items-center gap-1">
                            ...
                            {mySub?.final_score != null ? `AI GRADED • ${mySub.final_score} / ${t.points}` : 'AI GRADED'}
                          </span>
                        )}
                        {!isTeacher && isEvaluating && (
                          <span className="text-[10px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-full bg-indigo-100 text-indigo-800 border border-indigo-200 flex items-center gap-1 animate-pulse">
                        ...
                        <button
                          ...
                          className={`px-5 py-2.5 rounded-xl text-xs font-black shadow-sm transition-all cursor-pointer flex items-center gap-1.5 ${
                            isCompleted
                              ...
                              : 'bg-[#087477] hover:bg-[#159A9C] text-white shadow-[#087477]/20'
                          }`}
                        >
                          {isCompleted ? (
                            <>
                              <CheckCircle2 className="w-3.5 h-3.5" />
                              <span>View Results {mySub?.final_score != null ? `(${mySub.final_score}/${t.points})` : ''}</span>
                            </>
                          ) : isEvaluating ? (
                            <>
                              <Loader2 className="w-3.5 h-3.5 animate-spin" />
                              <span>AI is correcting...</span>
                            </>
                          ) : (
