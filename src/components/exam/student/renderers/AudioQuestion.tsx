...
export const AudioQuestionComponent: React.FC<AudioQuestionProps> = ({
  ...
  return (
    <div className="space-y-4 pt-2 max-w-2xl mx-auto">
      {/* Audio Track Player */}
      {question.audioUrl && (
        <div className="p-4 rounded-2xl border border-pink-500/40 bg-[#0b142c] shadow-md flex flex-col sm:flex-row items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-pink-500/20 border border-pink-400/40 flex items-center justify-center text-pink-300 shrink-0">
            <Volume2 className="w-5 h-5" />
          </div>
          <audio controls src={question.audioUrl} className="w-full accent-pink-500" />
        </div>
      )}
