    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
      <div className="w-full max-w-4xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-white">
          <div>
            <h2 className="text-2xl font-black text-[#0a213c]">Create Knowledge Bitz</h2>
            <p className="text-sm text-slate-600 font-semibold mt-0.5">AI-Assisted Content Generation Wizard</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar bg-slate-50/50">
          ...
          {step === 1 && (
            <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="text-center mb-8">
                <h3 className="text-xl font-black text-[#0a213c] mb-2">Content Parameters</h3>
                <p className="text-slate-600 font-semibold">Choose category, English level, and quantity to generate an AI content prompt.</p>
              </div>

              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-black text-[#0a213c] mb-2">Knowledge Category</label>
                  <select 
                ...
                <div>
                  <label className="block text-sm font-black text-[#0a213c] mb-3">Target CEFR Level</label>
                    ...
                    {CEFR_LEVELS.map(level => {
                      ...
                      return (
                        <button
                          ...
                          className={`py-3 rounded-xl font-black border-2 transition-all cursor-pointer text-sm ${
                            isActive 
                ...
                <div>
                  <label className="block text-sm font-black text-[#0a213c] mb-3">Batch Size (Quantity)</label>
                  <div className="flex flex-wrap gap-3 items-center">
                    {BATCH_SIZES.map(size => (
                      <button
                        key={size}
                        onClick={() => setBatchSize(size)}
                        className={`px-5 py-3 rounded-xl font-black border-2 transition-all cursor-pointer text-sm ${
                          batchSize === size 
                            ? 'border-[#026fc3] bg-blue-50 text-[#026fc3] shadow-xs' 
                            : 'border-slate-300 bg-white text-[#0a213c] hover:border-slate-400'
                        }`}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {batchSize === 'custom' && (
                        <input 
                          ...
                          className="w-24 border-2 border-[#026fc3] bg-white text-[#0a213c] rounded-xl px-3 py-3 focus:outline-none font-black text-sm"
                        />
                      )}
                    </div>
                  </div>
          ...
          {step === 2 && (
            <div className="h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="text-center mb-6">
                <h3 className="text-xl font-black text-[#0a213c] mb-2">Prompt Ready</h3>
                <p className="text-slate-600 font-semibold">Paste this prompt into Gemini, ChatGPT, or Claude. Copy the AI's JSON response for the next step.</p>
              </div>

              <div className="flex-1 relative bg-[#0a213c] rounded-2xl overflow-hidden flex flex-col border border-slate-800 shadow-inner">
                <div className="flex justify-between items-center px-4 py-3 bg-black/40 border-b border-white/10">
                  <span className="text-xs font-mono text-slate-400 font-bold">system_prompt.txt</span>
                  <button 
                    onClick={handleCopyPrompt}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg font-black text-sm transition-all cursor-pointer ${
                      promptCopied 
          ...
          {step === 3 && (
            <div className="h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="text-center mb-6">
                <h3 className="text-xl font-black text-[#0a213c] mb-2">Import AI Content</h3>
                <p className="text-slate-600 font-semibold">Paste the raw JSON response from the AI tool below to validate the contents.</p>
              </div>
              ...
              {jsonError && (
                <div className="mb-4 p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-black text-sm">Validation Error</h4>
                    <p className="text-sm mt-1 font-medium">{jsonError}</p>
                  </div>
                </div>
              )}
              ...
              <textarea
                ...
                className="flex-1 min-h-[300px] w-full border border-slate-300 bg-white text-[#0a213c] placeholder:text-slate-400 rounded-2xl p-6 font-mono text-sm focus:ring-2 focus:ring-[#026fc3]/25 focus:border-[#026fc3] outline-none resize-none shadow-inner font-semibold"
              />
            </div>
          )}
          ...
          {step === 4 && (
            <div className="h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="text-center mb-6">
                <h3 className="text-xl font-black text-[#0a213c] mb-2">Diagnostic Results</h3>
                <p className="text-slate-600 font-semibold">Review the validation results for the imported records.</p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex flex-col items-center justify-center text-center">
                  ...
                  <span className="text-2xl font-black text-emerald-800">{validCount}</span>
                  <span className="text-xs font-black text-emerald-700 uppercase tracking-wider">Valid</span>
                </div>
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex flex-col items-center justify-center text-center">
                  ...
                  <span className="text-2xl font-black text-amber-800">{warningCount}</span>
                  <span className="text-xs font-black text-amber-700 uppercase tracking-wider">Warnings</span>
                </div>
                <div className="bg-rose-50 border border-rose-200 rounded-2xl p-4 flex flex-col items-center justify-center text-center">
                  <AlertCircle className="text-rose-600 mb-2" size={24} />
                  <span className="text-2xl font-black text-rose-800">{errorCount}</span>
                  <span className="text-xs font-black text-rose-700 uppercase tracking-wider">Errors</span>
                </div>
              </div>
                ...
                {validatedRecords.map((record, idx) => (
                  <div 
                    ...
                    className={`p-4 rounded-xl border flex items-start gap-4 ${
                      record.status === 'valid' ? 'bg-white border-slate-200' :
                      record.status === 'warning' ? 'bg-amber-50/70 border-amber-200' :
                      'bg-rose-50/70 border-rose-200'
                    }`}
                  >
                    <div className="mt-1 shrink-0">
                      {record.status === 'valid' && <Check className="text-emerald-600" size={20} />}
                      {record.status === 'warning' && <AlertTriangle className="text-amber-600" size={20} />}
                      {record.status === 'error' && <AlertCircle className="text-rose-600" size={20} />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs font-mono bg-slate-100 text-slate-600 font-bold px-2 py-0.5 rounded">#{idx + 1}</span>
                      ...
                      {record.issues.length > 0 && (
                        <ul className="mt-2 space-y-1">
                          {record.issues.map((issue, i) => (
                            <li key={i} className={`text-sm flex items-start gap-1.5 font-semibold ${
          ...
          {step === 5 && (
            <div className="h-full flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              {!importResult ? (
                <>
                  <div className="text-center mb-6">
                    <h3 className="text-xl font-black text-[#0a213c] mb-2">Ready to Import</h3>
                    <p className="text-slate-600 font-semibold">Previewing {totalImportable} valid records to be imported as drafts.</p>
                  </div>
                    ...
                    {validatedRecords.filter(r => r.status !== 'error').map((record, idx) => (
                      <div key={idx} className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs">
                        <div className="flex items-start justify-between gap-4 mb-3">
                          <h4 className="font-black text-lg text-[#0a213c] leading-tight">{record.original.title}</h4>
                          <span className="shrink-0 text-xs font-black uppercase tracking-wider bg-blue-50 text-[#026fc3] border border-blue-100 px-2.5 py-1 rounded-md">
                            {record.original.topic_id}
                          </span>
                        </div>
                        <p className="text-sm text-slate-600 font-medium line-clamp-2 mb-4 leading-relaxed">
                          {record.original.short_fact}
                        </p>
                        <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                          <p className="text-xs font-black uppercase text-[#0a213c] mb-1.5">Quiz Preview:</p>
                          <p className="text-sm italic font-medium text-slate-700">"{record.original.quiz?.question}"</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center max-w-sm mx-auto">
                  <div className="w-20 h-20 bg-emerald-50 border border-emerald-200 rounded-full flex items-center justify-center mb-6">
                    <Check className="text-emerald-600 stroke-[3]" size={40} />
                  </div>
                  <h3 className="text-2xl font-black text-[#0a213c] mb-3">Import Complete!</h3>
                  <p className="text-slate-600 font-semibold mb-6">
                    Successfully imported <strong className="text-[#0a213c]">{importResult.success}</strong> Bitz as drafts.
        ...
        {/* Footer */}
        <div className="px-6 py-5 border-t border-slate-200 bg-white flex items-center justify-between mt-auto">
          ...
          <div className="flex items-center gap-3">
            {step > 1 && !importResult && (
              <button
                ...
                className="px-5 py-2.5 rounded-xl font-bold text-[#0a213c] hover:bg-slate-100 border border-slate-300 transition-colors flex items-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <ChevronLeft size={18} />
            ...
            {step === 1 && (
              <button
                onClick={() => setStep(2)}
                className="px-6 py-2.5 rounded-xl font-black bg-[#026fc3] hover:bg-[#025ea6] text-white shadow-md transition-all flex items-center gap-2 cursor-pointer"
              >
            ...
            {step === 3 && (
              <button
                ...
                className="px-6 py-2.5 rounded-xl font-black bg-[#026fc3] hover:bg-[#025ea6] text-white shadow-md transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            {step === 5 && importResult && (
              <button
                ...
                className="px-8 py-2.5 rounded-xl font-black bg-[#026fc3] hover:bg-[#025ea6] text-white shadow-md transition-all cursor-pointer"
              >
                Close
              </button>
            )}
