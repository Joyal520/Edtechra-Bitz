...
/**
 * Normalizes mixed HTML/Markdown lesson content:
 * - Unwraps <div>, <p>, <br> generated by contenteditable editors.
 * - Converts HTML headings (<h1>-<h4>) to Markdown (#, ##, ###, ####).
 * - Converts HTML lists (<ul>, <ol>, <li>) to Markdown (- item).
 * - Converts HTML formatting (<b>, <strong>, <i>, <em>, <s>, <del>) to Markdown (**, *, ~~).
 * - Converts HTML blockquotes to Markdown (> quote).
 * - Strips dangerous tags (<script>, <style>, <iframe>, <object>, <embed>).
 * - Preserves GFM tables, links, images, and plain text.
 */
export function normalizeLessonContent(raw: string): string {
  if (!raw) return '';
  let text = raw;

  // Unescape standard HTML entities if present
  text = text
    .replace(/&nbsp;/g, ' ')
    .replace(/&amp;/g, '&')
    .replace(/&quot;/g, '"')
  ...
  text = text.replace(/<(script|style|iframe|object|embed)\b[^<]*(?:(?!<\/\1>)<[^<]*)*<\/\1>/gi, '');
  ...
  // Convert closing </p> and </div> to newlines
  text = text.replace(/<\/p>/gi, '\n\n');
  text = text.replace(/<\/div>/gi, '\n');

  // Strip opening <p> and <div> wrappers
  text = text.replace(/<p[^>]*>/gi, '');
  text = text.replace(/<div[^>]*>/gi, '');

  // Strip empty font or span wrapper tags that don't have styles
  text = text.replace(/<span\s*>(.*?)<\/span>/gi, '$1');
  text = text.replace(/<font\s*>(.*?)<\/font>/gi, '$1');

  // Clean excessive consecutive newlines (more than 2)
  text = text.replace(/\n{3,}/g, '\n\n');
 ...
 * - Normalizes HTML wrappers from rich-text editors into clean Markdown/text.
 * - Renders GFM Markdown (headings, lists, quotes, tables) via MarkdownRenderer.
 * - Handles pure plain text seamlessly as comfortable reading paragraphs.
...
export const FormattedLessonText: React.FC<{
  ...
  // 1. Normalize content to remove HTML wrappers while preserving formatting as Markdown
  const normalized = normalizeLessonContent(text);

  // 2. If the normalized content still has complex rich HTML tags without markdown syntax, render via safe HTML pipeline
  if (isHtmlContent(normalized) && !/(^|\n)(#{1,6}\s|[-*+•]\s|\d+\.\s|>|\|.*\|)/.test(normalized)) {
    return renderSanitizedHtml(normalized, textScale, className);
  }

  // 3. Otherwise render via Markdown engine (handles headings, lists, tables, blockquotes, bold, italic, paragraphs)
  return (
    <MarkdownRenderer
      content={normalized}
      className={className}
