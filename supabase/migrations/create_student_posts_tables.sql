-- ============================================================================
-- EDTECHRA-BITZ: Student Posts, Moderation, Media Storage & Feed Migration
-- ============================================================================

-- 1. Create student_posts Table with All Moderation & Media Columns
CREATE TABLE IF NOT EXISTS public.student_posts (
  ...
  status TEXT DEFAULT 'approved' CHECK (status IN ('pending', 'approved', 'rejected', 'review')),
...
-- 2. Foreign Key relationship to public.profiles for PostgREST joins
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'fk_student_posts_profile'
  ) THEN
    ALTER TABLE public.student_posts
      ADD CONSTRAINT fk_student_posts_profile
      FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
  END IF;
END $$;

-- 3. Post Likes Table (Per-user persistent likes)
...
-- 5. High-Performance Feed Indexes
